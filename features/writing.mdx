...
description: "Write prose in Chronos Atlas using paginated rich-text notebooks with slash commands, entity @mentions, bubble toolbar formatting, and a full-screen Zen mode."
---
...
Notebooks can be exported as part of the world's complete **World Bible** compilation. This is handled by the Java backend's world-bible endpoint, which bundles notebook pages alongside entity data, timelines, and maps into a structured export package.

To export your data, go to **Settings → Sync & Backup** and use the **Export (ZIP)** button, which calls `SettingsUseCase.exportBackup()` and packages the full local SQLite database for download.
