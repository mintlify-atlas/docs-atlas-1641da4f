...
<CardGroup cols={2}>
  ...
  <Card title="Edges" icon="arrow-right-arrow-left">
    Each edge represents a `Relacion` record. The edge label displays the relationship type (e.g., "ally", "parent", "rules") drawn from the `tipo` field of the relationship.
  </Card>
</CardGroup>
...
export interface GraphNode {
  ...
  originalType: string;  // e.g. "PERSONAJE", "LUGAR", "ORGANIZACION"
  name: string;
  description: string;
  type: string;
  icon: string;
  x: number;
  y: number;
  color: string;        // derived from entity type for visual coding
}
```
...
- **Type filter** — filter the list by entity archetype (Individual, Collective, Territory, etc.) to reduce noise when working with large worlds.
...
| Control | Function |
|---|---|
| Entity type filter | Toggle visibility of nodes by type (Individual, Collective, Territory, Object, Event) |
