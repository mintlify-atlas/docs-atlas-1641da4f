---
title: "Contributing to Chronos Atlas"
...
<Note>
  Chronos Atlas is under **active development**. Phase 5 has several large modules planned — Visual Planning (Whiteboards with post-its and arc connectors), advanced Genealogy with complex lineages, a second-generation Timeline engine with custom fantasy calendars, an advanced Linguistics motor with conjugation rules and phonetic evolution, editorial compilation (EPUB, PDF, DOCX), and P2P collaboration via WebRTC. Check `Docs/03_Roadmap_Vivo.md` before starting any work in these areas to avoid duplicating effort or conflicting with in-progress implementations.
</Note>
