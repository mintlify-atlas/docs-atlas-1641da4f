...
description: "Explore how Chronos Atlas separates a thick React/SQLite frontend from a thin Java auxiliary backend, enabling fully offline operation with optional heavy-task support."
---
...
|---|---|---|
| `@features/*` | `src/features/*` | Accessing any feature's public API via its `index.ts` |
| `@components/*` | `src/features/Shared/*` | Importing specific Shared kernel modules (e.g. `@components/primitives`) |
| `@components` | `src/features/Shared/index` | Importing the full Shared public API barrel |
| `@context/*` | `src/features/App/context/*` | Application-level React contexts (active workspace, project ID, etc.) |
| `@domain/*` | Domain modules by feature | Typed domain contracts for `App`, `Maps`, `Timeline`, `Writing`, `Linguistics`, `WorldBible`, `Graph`, `Shell` |

<Note>
  Shared kernel imports use `@components` and `@components/*` — there is no `ui` segment in the path. Importing `@components/ui/Button` is incorrect; the right form is `@components/primitives/Button` (or whichever Shared subdirectory applies).
</Note>
